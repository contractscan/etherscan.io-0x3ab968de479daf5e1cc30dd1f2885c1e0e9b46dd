 ...
 */
contract Football is Context, IERC20, IERC20Metadata, AccessControl {
    ...
    event MinterAdded(address indexed operator, address indexed user);
     ...
     */
    function mint(address user, uint256 amount) public virtual onlyRole(minter) returns (bool) {
        _mint(user, amount);
        return true;
    }
     ...
     * And he can set minter
     */
    function addMinter(address to) public virtual onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
        address owner = _msgSender();
        _grantRole(minter, to);
        emit MinterAdded(owner, to);
        return true;
    }
